# Nihil Engine System Prompt — v1 (Protocol 1.0)

**Destination:** `packages/knowledge/prompts/` · consumed by engine mode 2 (BYOK/OmniRoute) in M1 Task 4.
**Status:** draft for Task 4 integration. Original text (CLAUDE.md rule) — patterns informed by `docs/source-mapping.md` §4 analysis, no copied content.

## Assembly notes (for the implementer — not part of the prompt)

The prompt is a template. The engine assembles it per turn with these slots:

| Slot | Content | Source |
|---|---|---|
| `{{FILE_TREE}}` | project file listing (relative paths) | `target.listFiles()` |
| `{{CONTEXT_FILES}}` | selected file contents with path headers | context selector (M1: all src files under budget; M2: smart selection) |
| `{{WORKFLOWS}}` | available workflow names + descriptions from `nihil.config.json` | `workflows.ts` |
| `{{TEMPLATE_NOTES}}` | stack facts for the active template | template metadata |
| `{{DESIGN_RULES}}` | design law block | `packages/knowledge/craft/` (M1: inline default below; M2: per-project DESIGN.md) |

Mode selection: send BUILD MODE or PLAN MODE prompt — never both. `<nihil-output>` blocks from the previous turn are injected into the next **user** message by the runner, before the user's text.

Token budget target: ~3.5–4K tokens assembled (excluding context files). Keep it lean — Lovable proves 20K works, v0 proves 46K works, but our protocol is simpler and instruction-following degrades with bloat.
...
```
You are Nihil, an expert full-stack engineer and product designer operating
...
## DEFAULT `{{DESIGN_RULES}}` BLOCK (M1 inline; M2 moves to craft/)

```
Design output must look intentional, not generated:
...
## DEFAULT `{{TEMPLATE_NOTES}}` (vite-react-shadcn)
...
```
You are Nihil in PLANNING mode. The user wants a plan reviewed before any
...
## Design rationale (traceability to Phase 0 analysis)

- **Discussion-first default, minimal-change discipline, batching** — Lovable's highest-leverage behaviors, restated originally.
- **Complete-file rule with explicit anti-elision language** — the single most damaging failure mode of write-tag protocols; bolt and dyad both legislate it. Ours is blunter because v1 has no fast-apply net.
- **Edit-vs-write decision rule + "when unsure, write"** — converts EDIT_NO_MATCH round-trips into one-shot successes; tuned for our exact apply chain (exact → fuzzy-ws → fail).
- **Feedback-loop section** — teaches the model what `<nihil-output>` is BEFORE it ever sees one; "never repeat a failed edit unchanged" targets the classic retry loop.
- **Named-workflows framing** ("you cannot run arbitrary shell") — states the boundary as a capability fact, not a rule to test (Replit's redirect pattern).
- **Design rules as a swappable slot** — v0's lesson (design quality is legislation) implemented the open-design way (content, not prompt text), so M2 can inject per-project DESIGN.md without touching the engine.
- **Plan-mode "trivial task" escape hatch** — prevents plan-mode theater for one-line changes.

## Open items for Task 4 integration

1. Context budget policy for `{{CONTEXT_FILES}}` in M1 (suggest: all of `src/` up to ~60K tokens, newest-modified first, with a hard cap warning) — M2 replaces with the context selector.
2. Whether `description` attributes should be required by prompt (recommended: yes for write/edit — they feed action cards and commit bodies).
3. Prompt-regression fixture: add a `prompts/__tests__/` golden test asserting the assembled prompt contains all five protocol rules and both example tags — cheap guard against template drift.
