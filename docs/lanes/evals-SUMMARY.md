...
## Merge integration notes (consolidation session, 2026-06-11)

- **Merged as partial.** This is the 30-item **protocol-mechanics** suite (6 categories ×
  5 items). The architecture review accepted it as a partial deliverable.
- **Complement pending.** The follow-up round — a rubric retrofit plus the
  **conversational**, **design-quality**, and **stress** categories — has **not** landed on
  the branch and is **not** included here. It remains an open task (see "Next steps" below
  and DECISIONS for the M2 eval-harness plan).
- **Lane-binding files dropped.** The root `AGENTS.md` / `BRIEF.md` (listed under
  "Deliverables produced" below) were lane-scoping artifacts, not owned content, and were
  **not** merged to main (ownership: `lane/evals` owns `evals/` only). This SUMMARY now
  lives at `docs/lanes/evals-SUMMARY.md`.
